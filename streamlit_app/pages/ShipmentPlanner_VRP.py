...
def generate_points(n: int, seed: int) -> List[Tuple[float, float]]:
    ...
    lats = rng.uniform(28.0, 48.5, n)  # approx US latitudes
    lons = rng.uniform(-122.0, -70.0, n)  # approx US longitudes
...
# Route Map
# ---------------------------------------------------------
def render_vrp_map(routes, depot, customers, demands):
    colors = [
    ...
    fig.add_trace(go.Scattergeo(
        ...
        text=[f"C{i:03d} | demand={demands[i]}" for i in range(len(customers))],
...
render_vrp_map(routes, depot_coord, customers_coord, demands)
